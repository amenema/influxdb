...
type mockTagValueStreamReader struct {
	responses []*datatypes.TagsResponse
}

func newMockTagValueStreamReader(responseValuess ...[]string) *mockTagValueStreamReader {
	responses := make([]*datatypes.TagsResponse, len(responseValuess))
	for i := range responseValuess {
		responses[i] = &datatypes.TagsResponse{
	...
	return &mockTagValueStreamReader{
		responses: responses,
	}
}

func (r *mockTagValueStreamReader) Recv() (*datatypes.TagsResponse, error) {
...
func TestStringIteratorStreamReader(t *testing.T) {
	...
	for _, tt := range tests {
		stream := newMockTagValueStreamReader(tt.responseValuess...)
